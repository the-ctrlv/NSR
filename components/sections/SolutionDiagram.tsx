      ...
      <Container>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_383px] lg:gap-16">
          <h2 id="solution-heading" className="font-serif text-4xl leading-[1.1] text-ink sm:text-h2">
            {solution.heading.map((line, i) => (
              <span key={line}>
                {line}{" "}
                {i < solution.heading.length - 1 && <br className="hidden lg:block" />}
              </span>
            ))}
          </h2>
          <div className="flex flex-col gap-3">
            <p className="font-serif text-lg leading-[1.3] text-ink lg:text-[22px]">{solution.paragraphs[0]}</p>
            <p className="font-sans text-base leading-[1.4] text-ink/80">{solution.paragraphs[1]}</p>
          </div>
        </div>
        ...
        >
          {solution.flow.map((step, i) => (
            <div key={step} className="flex flex-col items-center gap-6 sm:flex-row sm:gap-8 lg:gap-12">
              <span
              ...
              {i < solution.flow.length - 1 && (
                <span aria-hidden="true" className="h-10 w-px bg-hairline sm:h-px sm:w-16 lg:w-[88px]" />
              )}
            </div>
          ))}
        </Reveal>
